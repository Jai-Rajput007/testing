...
── Usage ────────────────────────────────────────────────────────────────────────
  python test_speech_models.py [interface] [--model MODEL_ID] [--keep-models] [--wakeword maya]

  python test_speech_models.py enP8p1s0
  python test_speech_models.py enP8p1s0 --model openai/whisper-base.en
  python test_speech_models.py enP8p1s0 --model UsefulSensors/moonshine-streaming-medium
  python test_speech_models.py enP8p1s0 --model vosk-model-en-us-0.22 --wakeword maya
...
── Voice Commands (while running) ───────────────────────────────────────────────
  "switch model whisper base"           → unload current, load whisper-base.en
  "switch model moonshine medium"       → unload current, load moonshine-streaming-medium
  "switch model vosk large"             → unload current, load vosk-model-en-us-0.22
...
DEFAULT_STT_MODEL = "UsefulSensors/moonshine-streaming-medium"
KEEP_MODELS       = False
WAKEWORD_MODEL    = None
...
while _i < len(_args):
    ...
    elif a == "--wakeword" and _i + 1 < len(_args):
        WAKEWORD_MODEL = _args[_i + 1]; _i += 2
    elif a == "--keep-models":
...
MAYA_ONNX_URL  = "https://raw.githubusercontent.com/Tuesday-Labs/maya/main/WAKEWORD/models/maya.onnx"
MAYA_ONNX_PATH = f"{MODELS_DIR}/maya.onnx"
...
class STTModel:
    ...
    def _load(self, model_id: str):
        ...
        config = AutoConfig.from_pretrained(model_id, cache_dir=cache, trust_remote_code=True)
        ...
        if self.family == "ctc":
            self.model = AutoModelForCTC.from_pretrained(
                model_id, cache_dir=cache, trust_remote_code=True, torch_dtype=self._dtype,
            )
    ...
    def _infer_granite(self, audio_np: np.ndarray) -> str:
        ...
        try:
            model_inputs = self.processor(text=prompt_text, audio=audio_np, return_tensors="pt", sampling_rate=SAMPLE_RATE)
        except Exception:
            try:
                model_inputs = self.processor(text=prompt_text, audios=audio_np, return_tensors="pt", sampling_rate=SAMPLE_RATE)
            except Exception:
                wav_tensor = torch.from_numpy(audio_np)
                model_inputs = self.processor(prompt_text, wav_tensor, return_tensors="pt", sampling_rate=SAMPLE_RATE)

        model_inputs = {k: v.to("cpu") for k, v in model_inputs.items() if isinstance(v, torch.Tensor)}
        duration       = len(audio_np) / SAMPLE_RATE
        max_new_tokens = max(int(duration * 8), 32)
        with torch.no_grad():
            outputs = self.model.generate(**model_inputs, max_new_tokens=max_new_tokens, do_sample=False, num_beams=1)
...
def handle_voice_commands(transcript: str, stt: STTModel) -> tuple:
    ...
    if "list model" in t or "what model" in t:
        cached = list_cached_models()
        current = stt.model_id.split("/")[-1]
        names = ", ".join(m.split("/")[-1] for m in cached) if cached else "None"
        speak(f"Current model: {current}. Cached on disk: {names}.")
        return stt, True

    if "switch model" in t or "load model" in t:
        ...
        try:
            ...
            print(f"[STT ERROR] Failed to load {new_id}: {e}")
...
def main():
    ...
    oww_model = None
    if WAKEWORD_MODEL:
        if WAKEWORD_MODEL.lower() == "maya":
            if not os.path.exists(MAYA_ONNX_PATH):
                print(f"[WAKEWORD] Downloading maya.onnx from Tuesday-Labs ...")
                urllib.request.urlretrieve(MAYA_ONNX_URL, MAYA_ONNX_PATH)
            
            print("[WAKEWORD] Loading openwakeword with maya.onnx ...")
            try:
                from openwakeword.model import Model as OWWModel
                import openwakeword
                openwakeword.utils.download_models()
                
                oww_model = OWWModel(wakeword_models=[MAYA_ONNX_PATH], inference_framework="onnx")
                print(f"[WAKEWORD] Maya wake word loaded successfully.")
            except ImportError:
                print("[WAKEWORD ERROR] openwakeword is not installed. Run: pip install openwakeword onnxruntime")
    ...
    while True:
        try:
            # ── Phase 1: Wake Word Standby (If Active) ──
            if oww_model:
            ...
            # ── Phase 2: Speech Recording & STT ──
            if not oww_model:
                print(f"\n[TEST] [{stt.model_id.split('/')[-1]}] Waiting for speech ...")
            else:
                print(f"[TEST] Listening for command ...")
